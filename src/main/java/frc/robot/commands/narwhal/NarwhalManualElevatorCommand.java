...
public class NarwhalManualElevatorCommand extends Command {
    ...
    private final Supplier<Boolean> wristPastHalfWaySupplier;
     ...
     */
    public NarwhalManualElevatorCommand(NarwhalElevator narwhalElevator, XboxController controller, Supplier<Boolean> wristPastHalfWaySupplier) {
        this.narwhalElevator = narwhalElevator;
        this.xboxController = controller;
        this.wristPastHalfWaySupplier = wristPastHalfWaySupplier;
        //addRequirements(narwhalElevator);
    }

    @Override
    public void execute() {
        if (narwhalElevator.currentState == NarwhalElevatorState.L4 || narwhalElevator.currentState == NarwhalElevatorState.L3){
            if(!wristPastHalfWaySupplier.get()){ // wrist is not ready for elevator to change position
                return;
            }
        }
