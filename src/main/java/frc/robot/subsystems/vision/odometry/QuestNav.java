...
public class QuestNav extends SubsystemBase {
  ...
  private Transform2d robotToQuestNav;
  private SwerveDrivePoseEstimator poseEstimator;

  /**
   * Creates a QuestNav object with the following offset
   * @param robotToQuestNav the offset transform of the quest
   * @param poseEstimator the shared pose estimator to feed data into
   */
  public QuestNav(Transform2d robotToQuestNav, SwerveDrivePoseEstimator poseEstimator) {
    this.robotToQuestNav = robotToQuestNav;
  ...
  public void setRobotPose(Pose2d pose) {
    setQuestPose(pose.transformBy(robotToQuestNav));
  }
   ...
   */
  private Rotation2d getQuestYaw() {
  ...
  public Pose2d getQuestPose() {
    return new Pose2d(getQuestTranslation(), getQuestYaw());
  }

  public Pose2d getRobotPose() {
    return getQuestPose().transformBy(robotToQuestNav.inverse());
  }
  ...
  /**
   * updates the odometry with recent measurements
   */
  public void updateOdometry() {
    if (getConnected() && getTrackingStatus()) { 

      poseEstimator.addVisionMeasurement(getRobotPose(), getTimestamp(), VisionConstants.QUESTNAV_STANDARD_DEVIATION);
